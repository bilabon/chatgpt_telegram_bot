...
async def get_or_create_user(update: Update) -> User:
    ...
    if not user:
        ...
        async with get_db() as conn:
            language_code = tuser.language_code or 'en'
            sql = """
            ...
            args = (tuser.username or tuser.id, tuser.first_name or '', tuser.id,
                    language_code, role_id, time_added)
            await conn.execute(sql, args)
            await conn.commit()
        user = await get_user_by_tid(tuser.id)
        await add_balance(user_id=user.id, tokens=FREE_TOKENS, language_code=language_code)
    return user
...
async def add_balance(user_id, tokens, language_code='en') -> None:
    time_added = datetime.now(timezone.utc).isoformat()
    if language_code in ['zh-hans', ]:
        tokens = 2500
